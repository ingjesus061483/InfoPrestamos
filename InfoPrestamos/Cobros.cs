...
namespace InfoPrestamos
{
    ...
    {
      PrestamoDTO PrestamoTransporte;
        ClienteHelp ClienteHelp;
        List<AmortizacionDTO> cuotas;
        AmortizacionDTO cuota;
      ClienteDTO ClienteTransporte;
      //CuotaTransporte CuotaTransporte;
        AmortizacionHelp CuotaHelp;
        PagoHelp PagoHelp;
        FormaPagoHelp formaPagoHelp;
        public Cobros(ClienteHelp _clienteHelp,AmortizacionHelp _cuotaHelp,
                      PagoHelp _pagoHelp,FormaPagoHelp _formaPagoHelp,
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            {
                Persona  = new Persona() ,
                Lst =ClienteHelp .TEntity .ToList().Select(x=>new Persona
                {
     ...
     //       dgCuotas.DataSource =CuotaTransporte.GetList(idprestamo);
        ...
        private void btnGuardar_Click(object sender, EventArgs e)
        {
            if(cuotas.Count ==0)
            {
       Helper.         Utilities.GetMessage("No hay cuotas en la lista de pagos", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return;
            }
        ...
        private void dgCuotas_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (col == 0)
            {
                ...
                cuota = CuotaHelp.TEntity.Where (x=>x.Id ==id).ToList ().Select(x => new AmortizacionDTO
                {
                    Id = x.Id,
                ...
                {
                Helper.    Utilities.GetMessage("La cuota a pagar ya se encuentra registrada", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                    return;
                }
                cuotas.Add(cuota);
            }
        ...
        {
            cuotas = new List<AmortizacionDTO>();
        }
    }
}
